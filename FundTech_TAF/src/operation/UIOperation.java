...
public class UIOperation {
	...
	//						Debug.VALUE = "Test Data : " + Debug.VALUE +"<br> Verified with : " + verifyText;
			...
			case "DOWNLOADFTPFILE":
                ...
                //Host##Port##username##Password##File path to download##Property to store File name##Property that will hold number of files having similar Name.
                if(parts.length >= 6){
                	if(parts.length == 7){
                	if (GlobalLib.copyFileFromServer(parts[0], Integer.parseInt(parts[1]), parts[2], parts[3], parts[4], downloadPath,parts[5],parts[6])) {
                        System.out.println("pass");
                        strResult = "Pass";
                    }
                	else{
                        ...
                        strResult = "Fail";
                		 ...
                		 if (GlobalLib.copyFileFromServer(parts[0], Integer.parseInt(parts[1]), parts[2], parts[3], parts[4], downloadPath,parts[5],"")) {
                         System.out.println("pass");
                         strResult = "Pass";
                     }
                		 else{
...
//				}
			...
			case "SAVEWINDOW":
				...
				propertyFileOperation.writeProperty( value + "," + winHandle);
				strResult = "Pass";
				break;
			...
			case "SETWINDOW":
				String winToSet= value;
				driver.switchTo().window(winToSet);
				strResult = "Pass";
				break;
			...
			*/
			...
			case "VERIFYFROMDBQUERY":
				...
				for (String part : parts) {
					...
					if (dbOperation.executeQueryAndVerify(subParts[1], subParts[2], subParts[0], dbPollingDuration, dbPollingMaxAttempt)) {
						...
						dbOperation.destroyConnection(value);//Closing DB connection after TC Fails
						strResult = "Fail";
			...
			break;
	         ...
	         case "VERIFYBYXPATH":
        	 	   ...
        	 	   for (String part : parts) {
        	 		  subParts = part.split(Separator.SEPARATOR_DOUBLE_HASH);
	   	        	   ...
	   	        	   if (subParts.length > 4 && subParts.length < 3) {
	   	        		Debug.traceMessage = Debug.traceMessage + "Invalid input";
	   	        		   ...
	   	        		   if (xpathValue.equals(xpathValueToVerify)) {
	   	        			   strResult = "Pass";
	   	        			   if(subParts.length == 4 && !("NULL".equalsIgnoreCase(subParts[3]))){
	   	        				propertyFileOperation.writeByKeyValueProperty(subParts[3], xpathValue.length()+"");
	   	        				System.out.println("Number of Characters in Xpath :- " + xpathValue.length());
	   	        				Debug.traceMessage = Debug.traceMessage +  "Number of Characters in Xpath :- " + xpathValue.length();
	   	        			   }
             ...
             case "FILEOPERATION":
              ...
              case "COPYFTPFILE":
                 parts = value.split(Separator.SEPARATOR_COMMA);
                 String fileNameToCopy = GlobalLib.getPropertyOrValue(parts[0]);
                 String fileToCopy  = "";
                 Debug.traceMessage = Debug.traceMessage + "File to Upload : " + fileToCopy + "<Br> Location to Copy : " + parts[6] ;
            //     Pain_001.001.03_Bulk_Positive${Prop_FileRef}.xml,/home/was/dh/Deoyani,192.168.168.37,22,was,was,
            //     /home/was/dh/profiles/SPPRDT_OM_WAS01/data/incoming/containers,192.168.168.37,22,was,was
                 if (parts.length == 11) {
                	 if (GlobalLib.copyFileFromServerToServer(parts[2], Integer.parseInt(parts[3]), parts[4], parts[5], parts[1], parts[7],Integer.parseInt(parts[8]),parts[9],parts[10],parts[6],parts[0])) {
                         System.out.println("pass");
                         strResult = "Pass";
                     }
                 } else {
		    ...
		    case "VARFINDNREPLACE":
		    	/*
				 *  Finds and replaces the specified property values in specified Variable to destination file/variable	
				 * Issue Fixed: Dependency of storing message files (.xml and .txt in Template Folder) on local machine
				 * 
				 * */
        	...
        	case "ARITHMETICOPERATION":
				parts = value.split(Separator.SEPARATOR_EQUALTO);
				String operend1="", operend2="",  optr= "";
				String subprt = parts[0];
				
				String operands[] = subprt.split("\\+|\\-|\\*|<|>|/|<|>|%");
				System.out.println("Arithmetic Operation operands[]:-"+Arrays.toString(operands));
				Object reslt = null;
				if(subprt.contains("+")){
					optr = "+";
				}else if(subprt.contains("-")){
					optr = "-";
				}else if(subprt.contains("*")){
					optr = "*";
				}else if(subprt.contains("/")){
					...
					optr = "%";
					 ...
					 System.out.println("Please Provide valid Arithmetic Operator(+, -, *, /, %)");
					 strResult = "Fail";
					 throw new ProvisoException("Invalid Arithmetic perator");
					}
				if((operands[0].startsWith("Prop")))
				{	
					operend1 =  GlobalLib.getPropertyOrValue(operands[0].split(Separator.SEPARATOR_UNDERSCORE, 2)[1]);
					
				}
				else{
					operend1 = operands[0];
				}
				if((operands[1].startsWith("Prop")))
				{	
					operend2 =  GlobalLib.getPropertyOrValue(operands[1].split(Separator.SEPARATOR_UNDERSCORE, 2)[1]);
				}
				else{
					operend2 = operands[1];
				}
				
				reslt = GlobalLib.arithmeticOperation(operend1,optr,operend2);
				System.out.println("Arithmetic Operation Result:-"+reslt);
				propertyFileOperation.writeByKeyValueProperty(parts[1], reslt.toString());
				strResult = "Pass";
				break;	
              	...
              	String restResponse = null;
              	 if(parts.length <= 5)              		 
              	 {
              		if(!parts[3].equalsIgnoreCase("NULL")){
              			...
              			String[] headers = parts[3].split(Separator.SEPARATOR_COMMA);
              			for(String header : headers){
                        	if(!header.split(Separator.SEPARATOR_COLON)[0].contains("Auth")){
                        		reqHeaders.put(header.split(Separator.SEPARATOR_COLON)[0],header.split(Separator.SEPARATOR_COLON)[1]);
                        	}
                        	else{
                        		if(!header.split(Separator.SEPARATOR_COLON)[0].contains("AuthUser")){
                        			user = header.split(Separator.SEPARATOR_COLON)[1];                        			
                        		}
                        		if(!header.split(Separator.SEPARATOR_COLON)[0].contains("AuthPwd")){
                        			pwd = header.split(Separator.SEPARATOR_COLON)[1];                        			
                        		}
              		...
              		case  "GET" :
              			restResponse = ExecuteREST.executeRestGETRequest(parts[0],reqHeaders,user,pwd);
              			propertyFileOperation.writeByKeyValueProperty(parts[4], restResponse);
                   		strResult = "Pass";
              		break;
              		
              		case  "POST" :
              			restResponse = ExecuteREST.executeRestPOSTRequest(parts[0],parts[1],reqHeaders,user,pwd);
              			String respStr = null;
              			
              			 if(null != restResponse){
              				if(restResponse.startsWith("[")){
              					respStr = restResponse.substring(1, restResponse.lastIndexOf("]"));
              					//respStr = "{"+restResponse+"}";
              					propertyFileOperation.writeByKeyValueProperty(parts[4], respStr);
              					strResult = "Pass";
                  			}
              				else {
              					propertyFileOperation.writeByKeyValueProperty(parts[4], restResponse);
              				 ...
              				 System.out.println("Getting empty rest response.");
              				strResult = "Fail";
              			 }
              		break;
              		
              		case  "PUT" :
              			restResponse = ExecuteREST.executeRestPUTRequest(parts[0],parts[1],reqHeaders,user,pwd);
              			 if(null != restResponse){
              				 propertyFileOperation.writeByKeyValueProperty(parts[4], restResponse);              			 	
              				 strResult = "Pass";
              			 }else{
              		...
              		case  "DELETE" :
              			restResponse = ExecuteREST.executeRestDELETERequest(parts[0],reqHeaders,user,pwd);
                   		strResult = "Pass";
              		break;
              		
              		
              		default:
              			System.out.println("Invalid REST Method");
              			strResult = "Fail";
              		}
              	}
              	else {
        	...
        	case "VERIFY_REST_STATUS_CODE":
        		if(ExecuteREST.restResponseStatusCode != null){
	        		if(value.equals(ExecuteREST.restResponseStatusCode)){
	        			System.out.println("Rest Status Code verified successfully");
	        			System.out.println("Expected Value : "+ExecuteREST.restResponseStatusCode+"  Actual Value : "+value);
	        			strResult = "Pass";
	        		}else{
	        			System.out.println("Failed to verify Rest Status Code");
	        			System.out.println("Expected Value : "+ExecuteREST.restResponseStatusCode+"  Actual Value : "+value);
	        			strResult = "Fail";
	        		} 
        		}else{
        			System.out.println("Rest Request has not been executed successfully");
        			strResult = "Fail";
        		}
        		break;
        	
        	case "VERIFY_REST_RESPONSE_HEADERS":
        		parts = value.split(Separator.SEPARATOR_DOUBLE_HASH);        		
        		HashMap<String, String> requestHeaders = new HashMap<String, String> ();
        		for(String header : parts){
        			requestHeaders.put(header.split(Separator.SEPARATOR_COLON)[0], header.split(Separator.SEPARATOR_COLON)[1]);
        		}
        		Iterator<Header> itr = ExecuteREST.restResponseHeaders;
        		HashMap<String, String> responseHeaders = new HashMap<String, String> ();
        		boolean headerNotFound = false;
        		if(itr != null){
        			while(itr.hasNext()){
    					Header header = itr.next();
    					responseHeaders.put(header.getName().toString(), header.getValue().toString());
    				}
        			for(String key : requestHeaders.keySet()){        				
	        			if(responseHeaders.containsKey(key)){
			        		if(responseHeaders.get(key).contains(requestHeaders.get(key))){
			        			System.out.println("Rest response headers verified successfully for header : "+key);
			        			System.out.println("Expected Value : "+requestHeaders.get(key)+"  Actual Value : "+responseHeaders.get(key));
			        			Debug.traceMessage = Debug.traceMessage  + "Expected Value : "+requestHeaders.get(key)+"  Actual Value : "+responseHeaders.get(key);
			        		}else{
			        			System.out.println("Failed to verify Rest response headers for header : "+key);
			        			System.out.println("Expected Value : "+requestHeaders.get(key)+"  Actual Value : "+responseHeaders.get(key));
			        			Debug.traceMessage = Debug.traceMessage  + "<br> Expected Value : "+requestHeaders.get(key)+"  Actual Value : "+responseHeaders.get(key);
			        			headerNotFound = true;
			        		}
	        			}else{
	        				System.out.println("Rest response not contains the header : "+key);
		        			Debug.traceMessage = Debug.traceMessage  + "<br> Rest response not contains the header : "+key;
        			...
        			System.out.println("Rest response does not contains any headers");
        			strResult = "Fail";
        		}
        		break;
        		
        	case "VERIFY_REST_ATTRIBUTES":      
                /**
                 * Verify Rest attributes with response attributes.
                 * @param Response Object 
                 * @param Object containing Attributes that need to verify
                 * @param comaprison operator (=,!=) 
                 */
                 parts = value.split(Separator.SEPARATOR_DOUBLE_HASH);
                 
            	boolean compareResult = ExecuteREST.verifyRESTAttributes(parts[0],parts[1],parts[2]);
         			if(compareResult){
         				strResult = "Pass";
         			}else{
         				strResult = "Fail";
        	...
        	case "GET_REST_ATTRIBUTES":      
                /**
                 * Store specified attribute to a variables
                 * @param Response Object 
                 ...
                 */
                 ...
                 boolean writeAttr = ExecuteREST.writeAttributeToVariable(parts[0],parts[1]);
         			if(writeAttr){
         				strResult = "Pass";
         			}else{
         				strResult = "Fail";
        	...
        	case "GET_REST_ATTRIBUTES_COUNT":      
                /**
                 * Get rest attribute count
                 * @param Response Object 
                 * 
                 * 
                 */
                 ...
                 int countAttr = ExecuteREST.countNumberOfAttributes(parts[0],parts[1]);
         			if(countAttr > 0){
         				propertyFileOperation.writeByKeyValueProperty(parts[1],countAttr+"");	
         				strResult = "Pass";
         			}else{
         				strResult = "Fail";
         			}
        	...
        	case "GET_CHAR_COUNT":      
                /**
                 * Get the length of specified string/property 
                 * @param Property/String object  
                 * @param Property to store the length
                 * 
                 */
               /*  parts = value.split(Separator.SEPARATOR_DOUBLE_HASH);
                
                 int strLen = GlobalLib.getObjectLength(parts[0]);
         			if(strLen == 0){
         				strResult = "Fail";
         			}else{
         				strResult = "Pass";
         			}
         			System.out.println("Number of charactrers Present in Given Property :-"+strLen);
         			Debug.traceMessage = Debug.traceMessage  + "<br><br> Number of charactrers Present in Given Property :-"+strLen;
    				propertyFileOperation.writeByKeyValueProperty(parts[1],strLen+"");	*/
        		  parts = value.split(Separator.SEPARATOR_FOUR_HASH);
       	 	   for (String part : parts) {
       	 		  subParts = part.split(Separator.SEPARATOR_DOUBLE_HASH);
	   	        	   String xpathValueToVerify = GlobalLib.getPropertyOrValue(subParts[0]);
